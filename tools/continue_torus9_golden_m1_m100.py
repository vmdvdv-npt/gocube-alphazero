...
def _locked(name: str) -> ArenaPolicyError:
    return ArenaPolicyError(
        f"{name} belongs to the frozen Torus9 Arena runner; "
        f"use {FROZEN_ARENA_OVERRIDE_FLAG} only for historical reproduction"
    )
...
def _dispatch_frozen() -> None:
    ...
    sys.argv = [
        ...
        if argument != FROZEN_ARENA_OVERRIDE_FLAG
    ]
    try:
...
def main() -> None:
    ...
    if "--help" in arguments and FROZEN_ARENA_OVERRIDE_FLAG not in arguments:
        print(
            ...
            "Current production Arena: tools/arena.py --profile torus9"
        )
        return
    if FROZEN_ARENA_OVERRIDE_FLAG not in arguments:
        raise SystemExit(
            "LOCKED: historical Torus9 Arena runner. "
            "Use tools/arena.py --profile torus9 for current production. "
            f"For deliberate reproduction only, add {FROZEN_ARENA_OVERRIDE_FLAG}."
        )
    _dispatch_frozen()
