...
def _write_profile(root: Path) -> tuple[str, str]:
    ...
    path.write_text(json.dumps({"profile_id": "torus9-golden-current-v3", "profile_fingerprint": fingerprint, "self_play": {"games_per_iteration": 37}}), encoding="utf-8")
...
def _payload(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "arena": {
            ...
            "startset": {"master_seed": 404, "pairs": 9}
        },
        "health": {
            "poll_seconds": 7,
        ...
        "soft_stop": {"default_minutes": 47, "minimum_minutes": 13, "maximum_minutes": 91},
        "performance": {"checks": [{"metric": "moves_per_sec", "baseline": 17.25, "warning_ratio": 0.81, "fail_ratio": 0.62, "policy": "warning"}]},
        "learning": {"metrics": ["loss.total"], "stall_checks": [{"kind": "generation", "metric": "learning.samples_consumed_total", "window": 4, "minimum_delta": 123, "direction": "increase", "policy": "warning"}]},
...
def test_arena_cadence_is_not_hard_limited_to_five_through_ten(tmp_path: Path):
    payload = _payload(tmp_path)
