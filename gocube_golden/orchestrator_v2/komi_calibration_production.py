...
def aggregate_candidate_batches(
    ...
    """Return one cumulative candidate result from ordered valid Arena batches."""
    if not batches:
        raise KomiCalibrationError("cannot aggregate an empty komi calibration")

    ordered = sorted(
        (deepcopy(dict(batch)) for batch in batches),
        key=lambda item: int(item.get("batch", 0)),
    )
    expected_batch = 1
    total_games = total_valid = total_black = total_white = total_draws = 0
    margins: list[object] = []
    startset_fingerprint: str | None = None
    ...
    for evidence in ordered:
        ...
        if batch_number != expected_batch:
            raise KomiCalibrationError(
                "komi calibration batch evidence is not contiguous"
            )
        expected_batch += 1
        ...
        if not isinstance(stats, Mapping) or not isinstance(identity, Mapping):
            raise KomiCalibrationError("komi calibration batch evidence is malformed")
        if evidence.get("validity") != "VALID":
            raise KomiCalibrationError("komi calibration batch is not VALID")
        technical = int(stats.get("technical_games", 0) or 0)
        invalid = int(stats.get("invalid_games", 0) or 0)
        if technical or invalid:
            raise KomiCalibrationError(
                "komi calibration aggregate contains technical/invalid outcomes"
            )
        ...
        if valid <= 0 or black + white + draws != valid or games != valid:
            raise KomiCalibrationError(
                "komi calibration batch counters are inconsistent"
            )

        startset = identity.get("startset")
        if not isinstance(startset, Mapping):
            raise KomiCalibrationError(
                "komi calibration batch lacks frozen startset identity"
            )
        fingerprint = str(startset.get("fingerprint", ""))
        if not fingerprint:
            raise KomiCalibrationError(
                "komi calibration batch lacks startset fingerprint"
            )
        if startset_fingerprint is None:
            startset_fingerprint = fingerprint
        elif startset_fingerprint != fingerprint:
            raise KomiCalibrationError(
                "komi calibration batches use different startsets"
            )
        ...
        evaluation_ids.append(str(evidence.get("evaluation_id", "")))
        evaluation_fingerprints.append(
            str(evidence.get("evaluation_fingerprint", ""))
        )
    ...
    result.update(
        {
            ...
            "evaluation_fingerprint": sha256_fingerprint(
                {
                    "kind": "komi-calibration-cumulative-v1",
                    "batch_fingerprints": evaluation_fingerprints,
                }
            ),
            ...
            "stats": {
                ...
                "confidence_interval": _wilson_interval(total_black, total_valid),
...
class ProductionKomiCalibrationRunnerV2(_BaseKomiCalibrationRunnerV2):
    ...
    def _select_or_extend(
        ...
        ambiguous = (
            abs(initial_biases[1.5] - initial_biases[2.5])
        ...
        if ambiguous:
            ...
            return 1.5 if cumulative_biases[1.5] <= cumulative_biases[2.5] else 2.5
        ...
        return 1.5 if initial_biases[1.5] <= initial_biases[2.5] else 2.5
