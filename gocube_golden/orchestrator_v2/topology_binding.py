...
This is the single Orchestrator-level registry allowed to select a scientific
production bridge, Arena profile, or deterministic evaluation startset.
Lifecycle code stays topology-neutral and unknown topologies fail closed.
"""
...
def _cube_arena_search(config: EffectiveConfig):
    ...
    return CubeArenaSearchConfig(
        simulations=int(
            _required(arena, ("simulations", "mcts_simulations"), "Cube Arena simulations")
        ),
        cpuct=float(_required(arena, ("cpuct",), "Cube Arena cpuct")),
        fpu=float(_required(arena, ("fpu",), "Cube Arena fpu")),
        watchdog=int(
            _required(arena, ("watchdog", "technical_move_limit"), "Cube Arena watchdog")
        ),
    )
...
class TopologyBinding:
    ...
    def default_arena_profile(self, config: EffectiveConfig) -> str:
        ...
        return CubeV2ArenaProfile(
            size=int(self.cube_size),
            search_config=_cube_arena_search(config),
        ).profile_id
    ...
    def validate_arena_profile(self, profile: str, config: EffectiveConfig) -> str:
        ...
        if profile != expected:
            raise ValueError(
                f"Arena profile {profile!r} is not compatible with {self.topology}; "
