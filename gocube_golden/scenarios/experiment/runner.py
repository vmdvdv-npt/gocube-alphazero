...
from ...orchestrator_v2.arena_runner import ArenaRunRequest, ArenaRunResult, ArenaRunnerV2, torus9_startset_ref
from ...orchestrator_v2.artifact_resolver import ArtifactResolver, ResolvedCheckpointNode, ResolvedEffectiveConfig
from ...orchestrator_v2.contracts import CheckpointRef, EvaluationIdentity, StartsetRef
from ...orchestrator_v2.experiment_plan import (
...
from ...orchestrator_v2.production_generation import ProductionTrainOne
...
class ExperimentRunnerV2:
    ...
    def __init__(
        ...
        *,
        ...
        if hasattr(self.arena_runner, "event_sink") and event_sink is not None:
            setattr(self.arena_runner, "event_sink", self._event_sink)
    ...
    def _stage_arena(
        ...
        *,
        ...
        self._notify_operator(
                ...
                f"execution={str(arena_result.execution_code_commit or '')[:12] or 'unknown'}.",
    ...
    def _validate_decision(
        ...
        try:
            evidence_wld = arena.wld
        except (TypeError, ValueError) as exc:
            raise ExperimentRunnerError(f"stage {stage} Arena has malformed W/L/D evidence") from exc
        if decision.wld != evidence_wld:
            raise ExperimentRunnerError(
                f"persisted stage {stage} winner decision disagrees with Arena W/L/D evidence"
            )
