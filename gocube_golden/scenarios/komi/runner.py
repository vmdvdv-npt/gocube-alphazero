...
from ...orchestrator_v2.arena_runner import ArenaRunRequest, ArenaRunResult, ArenaRunnerV2
from ...orchestrator_v2.artifact_resolver import ArtifactResolver, ResolvedCheckpointNode, ResolvedEffectiveConfig
from ...orchestrator_v2.generation_runner import OutputLineage
from ...orchestrator_v2.experiment_runner import LineageFactory
from ...orchestrator_v2.torus9_production import Torus9ProductionLineage
from ...orchestrator_v2.version import ORCHESTRATOR_VERSION
from ...torus9_contract import TORUS9_ALLOWED_KOMI
from ...notifications import coerce_event_sink, operator_event
from .policy import wilson_interval
...
def frozen_calibration_startset_ref(*, master_seed: int, games: int = 2048):
    ...
    from ...orchestrator_v2.contracts import StartsetRef
...
def _wilson_interval(successes: int, total: int) -> list[float] | None:
    """Historical private seam; statistics are owned by komi policy."""
...
def _result_from_state(raw: Mapping[str, object]) -> ArenaRunResult:
    from ...orchestrator_v2.contracts import EvaluationIdentity
...
class KomiCalibrationRunnerV2:
    ...
    def __init__(
        ...
        *,
        ...
        self.notifier = notifier  # compatibility attribute only
    ...
    def run(self) -> KomiCalibrationResult:
        ...
        if state.get("state") == COMPLETE_HANDOFF:
            ...
            self._notify(
                "CALIBRATION_HANDOFF_COMPLETED",
                f"Komi calibration handoff completed for {self.config.resolved_child_lineage_id}.",
                f"handoff:{self.config.resolved_child_lineage_id}:{state.get('selected_komi', 'unknown')}",
            )
        ...
        if state.get("state") == TRAINING_RESUMED:
            ...
            self._notify(
                "CALIBRATION_HANDOFF_COMPLETED",
                f"Komi calibration handoff completed for {self.config.resolved_child_lineage_id}.",
                f"handoff:{self.config.resolved_child_lineage_id}:{selected:g}",
            )
        return self._result(state, parent)
    ...
    def _run_candidate(
        ...
        *,
        ...
        self._notify(
            ...
            f"(execution={str(result.execution_code_commit or '')[:12] or 'unknown'})",
    ...
    def _notify(self, key: str, message: str, suffix: str) -> None:
        if not getattr(self._event_sink, "structured", False) and self.notifier is None:
            return
        try:
            self._event_sink.publish(
                operator_event(
                    ...
                    owner_id=self.config.calibration_id,
                    action_id=f"{self.config.calibration_id}:{suffix}",
                    message=message,
                    payload={"calibration_id": self.config.calibration_id},
                    correlation_id=self.config.calibration_id,
                )
            )
        except Exception:
...
def __getattr__(name: str):
    if name == "ProductionKomiCalibrationRunnerV2":
        from .production_runner import ProductionKomiCalibrationRunnerV2
        return ProductionKomiCalibrationRunnerV2
    if name in {"aggregate_candidate_batches", "select_komi"}:
        from .policy import aggregate_candidate_batches, select_komi
        return {"aggregate_candidate_batches": aggregate_candidate_batches, "select_komi": select_komi}[name]
