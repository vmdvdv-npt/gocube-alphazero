...
def _next_state(state: GoldenState, *, stones: tuple[Stone,...], side_to_move: Stone,
                history: tuple[BoardKey,...], consecutive_passes: int) -> GoldenState:
    return GoldenState(
        stones=stones, side_to_move=side_to_move, superko_history=history,
        consecutive_passes=consecutive_passes, topology=state.topology,
        rules_id=state.rules_id, rules_fingerprint=state.rules_fingerprint, komi=state.komi,
...
def apply_action(state: GoldenState, action: int | str) -> Transition:
    ...
    after=_next_state(state, stones=new_stones, side_to_move=opponent(moving_color),
                      history=state.superko_history+(new_key,), consecutive_passes=0)
