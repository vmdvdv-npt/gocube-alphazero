...
This module is the only supported entry point for future Torus9 production
Arena execution.  The historical process Arena is exposed only as an explicit
foundation/reproduction path until central GPU inference brokering is restored.
"""
...
def run_torus9_process_foundation(
    *,
    ...
    This is intentionally not the current Golden production Arena. CUDA is
    rejected here because the historical implementation loads both models into
    every worker process; the next production implementation must instead use
    one central GPU inference owner.
    """
    ...
    # Lazy import keeps the frozen engines out of normal production import paths.
    from .torus9 import run_torus9_arena
