...
from libc.math cimport sqrt, log, atan, pow
...
_SQRT_THREE = 1.7320508075688772
...
cdef class MCTS:
    ...
    cdef public bint _root_noise_enabled
    ...
    cdef float _tdist3_cdf(self, float z):
        cdef float value = 0.5 + (
            atan(z / _SQRT_THREE) + _SQRT_THREE * z / (z * z + 3.0)
        ) / _PI
        if value < 0.0:
            return 0.0
        if value > 1.0:
    ...
    cdef float _katago_child_value_factor(self, float child_weight, float child_self_utility, float simple_value):
        ...
        probability = self._tdist3_cdf(z) + 0.0001
    ...
    cdef void _recompute_katago_node(self, Node node):
        ...
        cdef float max_child_weight = 0.0
        cdef float amount_to_subtract = 0.0
        cdef float amount_to_prune = 0.0
        cdef float post_prune_weight
        ...
        for c in node._children:
            ...
            if raw_child_weight > max_child_weight:
                max_child_weight = raw_child_weight
        ...
        # Pinned recomputeNodeStats uses chosenMoveSubtract/Prune during root
        # value aggregation only when root Dirichlet noise is enabled.
        if node is self._root and self._root_noise_enabled:
            amount_to_subtract = min(self.chosen_move_subtract, max_child_weight / 64.0)
            amount_to_prune = min(self.chosen_move_prune, max_child_weight / 64.0)
        ...
        for c in node._children:
            if c.n <= 0 or c.weight_sum <= 0.0:
                continue
            raw_child_weight = c.weight_sum
            if raw_child_weight < amount_to_prune:
                continue
            post_prune_weight = raw_child_weight - amount_to_subtract
            if post_prune_weight <= 0.0:
                continue
            child_self_utility = c.q if node.player == 1 else -c.q
            factor = self._katago_child_value_factor(raw_child_weight, child_self_utility, simple_value)
            adjusted_sum += post_prune_weight * factor
        if adjusted_sum <= 0.0:
            raise RuntimeError('KataGo value weighting removed every child')
        ...
        for c in node._children:
            ...
            child_self_utility = c.q if node.player == 1 else -c.q
            factor = self._katago_child_value_factor(raw_child_weight, child_self_utility, simple_value)
            desired_weight = post_prune_weight * factor * normalize
    ...
    cpdef void process_search_results(
        ...
        if was_terminal:
            ...
            if self._curnode == self._root:
                ...
                self._root_noise_enabled = bool(add_root_noise)
                if add_root_noise:
                    self._add_root_noise()
    ...
    cpdef np.ndarray probs(self, object gs, float temp=1.0, object apply_lcb=None):
        ...
        if self._katago_search and not self._force_legacy_search:
            if apply_lcb is None:
                # Pinned self-play move temperatures are strictly below 1,
                # policy supervision uses temp=1, and deterministic Arena uses
                # temp=0. This reproduces KataGo's self-play-only LCB disable
                # for the played move while retaining LCB elsewhere.
