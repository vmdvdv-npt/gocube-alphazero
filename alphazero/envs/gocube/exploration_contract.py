...
def root_policy_temperature(
    ...
    *,
    ...
    """Pinned KataGo root-policy temperature schedule."""
...
def chosen_move_temperature(
    ...
    *,
    ...
    """Pinned KataGo chosen-move temperature schedule."""
...
def chosen_move_temperature_scaling(
    ...
    *,
    ...
    """Framework adapter for SelfPlayAgent/MCTSPlayer temperature callbacks.

    A zero current temperature is an explicit deterministic-evaluation override
    (Arena), so preserve it rather than applying the self-play schedule.
    """
...
def retrospectively_reduce_root_weights(
    ...
    *,
    ...
    The production GoCube tree has no transpositions, so a child's edge-visit
    ratio is 1 and KataGo ``getChildWeight(edgeVisits)`` is simply the child's
    aggregated ``weightSum``. Unlike the legacy compatibility helper above,
    this preserves floating search weights rather than rounding to visits.
    """
...
def kata_value_child_weights(
    ...
    *,
    ...
    """Pinned KataGo bad-child downweighting used during node-value aggregation."""
    ...
    if total <= 0.0 or exponent == 0.0:
        return result

    simple_value = float(np.dot(result, utilities) / total)
    adjusted_sum = 0.0
    for idx, weight in enumerate(result):
        ...
        probability = 0.5 * (1.0 + math.erf(z / math.sqrt(2.0))) + 0.0001
        result[idx] = float(weight) * (probability ** float(exponent))
        adjusted_sum += result[idx]
    if adjusted_sum <= 0.0:
        return np.maximum(weights, 0.0)
    result *= total / adjusted_sum
    return result
    ...
    *,
    ...
    for idx in active:
        ...
        if edge <= 0.0 or stats_sum <= 0.0 or stats_sq <= 0.0:
            continue
        # GoCube's production search is a tree (no transpositions), so the
        # edge-visit ratio in KataGo getChildWeight(edgeVisits, childVisits) is
        # exactly 1. Preserve the child's aggregated weight statistics.
