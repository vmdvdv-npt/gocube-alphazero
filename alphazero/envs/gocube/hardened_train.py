...
def build_hardened_training_args(cli):
    ...
    # MCTSPlayer is used for checkpoint Arena and applies temp_scaling_fn on
    # every move. A fixed zero callback is required so deterministic Arena
    # cannot be raised to the legacy 0.2 minimum at periodic turn boundaries.
    args.temp_scaling_fn = const_temp_scaling
    return game_cls, args


def assert_resumable_run(args):
    existing_paths = [path for path in run_paths(args) if os.path.exists(path)]
    checkpoint_folder = os.path.join(args.checkpoint, args.run_name)
    checkpoints = glob(os.path.join(checkpoint_folder, "iteration-*.pkl"))
    if not existing_paths:
        raise RuntimeError("--allow-existing-run requires an existing hardened run namespace")
    if not checkpoints:
        raise RuntimeError(
            "Refusing resume: run namespace exists but contains no checkpoint. "
            "Use a fresh run name instead."
        )
