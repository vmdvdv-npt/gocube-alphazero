...
from .errors import CheckpointIncompatible, GenerationFailed, InvalidMoveHistory, InvalidRequest, TerminalPosition
...
class GoldenPositionContract:
    ...
    def __post_init__(self) -> None:
        ...
        if self.rule_set != GOLDEN_PROTOCOL_RULESET:
            raise InvalidRequest(f"Unsupported ruleSet {self.rule_set!r}; expected {GOLDEN_PROTOCOL_RULESET!r}")
...
class GoldenInteractiveSearchContract:
    ...
    def settings(self, mcts_sims: int) -> SearchSettings:
        ...
        return SearchSettings(simulations=mcts_sims, cpuct=self.cpuct, fpu=self.fpu, root_noise=self.root_noise, fast_search=self.fast_search, resign=self.resign, root_policy_temperature=self.root_policy_temperature, move_temperature=self.move_temperature, deterministic_tie_break=self.deterministic_tie_break)
...
def initial_state_for_position(position: GoldenPositionContract) -> GoldenState:
    ...
    raise InvalidRequest(f"No Golden initial state is registered for {position.topology} size {position.size}")
...
def _replay_actions(*, position: GoldenPositionContract, moves: Sequence[Mapping[str, object]]) -> tuple[GoldenState, tuple[int, int], tuple[tuple[str, ...], ...]]:
    ...
    for expected_number, move in enumerate(moves, start=1):
        ...
        if isinstance(move_number, bool) or not isinstance(move_number, int) or move_number != expected_number:
        ...
        try:
            ...
            raise InvalidMoveHistory(f"History move {expected_number} is illegal or unmappable: {exc}") from exc
        if action != PASS:
...
def replay_action_history(*, topology: str, size: int, rule_set: str, komi: float, moves: Sequence[Mapping[str, object]]) -> GoldenState:
    position = GoldenPositionContract(topology=topology, size=size, rule_set=rule_set, komi=komi)
    state, _captures, _captured_by_move = _replay_actions(position=position, moves=moves)
    return state


def replay_protocol_moves_strict(*, topology: str, size: int, rule_set: str, komi: float, moves: Sequence[Mapping[str, object]]) -> tuple[GoldenState, tuple[int, int]]:
    position = GoldenPositionContract(topology=topology, size=size, rule_set=rule_set, komi=komi)
    state, captures, captured_by_move = _replay_actions(position=position, moves=moves)
    for expected_number, (move, expected_captured) in enumerate(zip(moves, captured_by_move), start=1):
        captured = move.get("captured")
        if not isinstance(captured, list) or tuple(captured) != expected_captured:
            raise InvalidMoveHistory(f"Protocol captures disagree with Golden transition at move {expected_number}")
    return state, captures


def validate_checkpoint_position_compatibility(*, position: GoldenPositionContract, state: GoldenState, descriptor: CheckpointDescriptor, mapping: GoldenProtocolMapping | None = None) -> GoldenProtocolMapping:
    checks = {"topology": (descriptor.topology, position.topology), "size": (descriptor.size, position.size), "ruleSet": (descriptor.rule_set, position.rule_set), "komi": (descriptor.komi, position.komi)}
    for field, (actual, requested) in checks.items():
        if actual != requested:
            raise CheckpointIncompatible(f"Golden checkpoint {descriptor.checkpoint_id} is incompatible for {field}: checkpoint={actual!r}, requested={requested!r}")
    resolved_mapping = mapping or mapping_for_position(position)
    for field in ("architecture_id", "rules_fingerprint", "observation_fingerprint", "target_fingerprint"):
        value = getattr(descriptor, field)
        if not isinstance(value, str) or not value:
            raise CheckpointIncompatible(f"Golden checkpoint {descriptor.checkpoint_id} is missing {field}")
    contract = serving_contract(descriptor)
    expected_contract = {"topology": position.topology, "size": position.size, "rule_set": position.rule_set, "komi": position.komi, "architecture_id": descriptor.architecture_id, "rules_fingerprint": descriptor.rules_fingerprint, "observation_fingerprint": descriptor.observation_fingerprint, "target_fingerprint": descriptor.target_fingerprint}
    for key, expected in expected_contract.items():
        if contract.get(key) != expected:
            raise CheckpointIncompatible(f"Golden checkpoint {descriptor.checkpoint_id} serving contract mismatch for {key}")
    if contract.get("topology_fingerprint") != resolved_mapping.golden_topology_fingerprint:
        raise CheckpointIncompatible(f"Golden checkpoint {descriptor.checkpoint_id} topology fingerprint does not match the Golden mapping")
    if descriptor.rules_fingerprint != state.rules_fingerprint:
        raise CheckpointIncompatible(f"Golden checkpoint {descriptor.checkpoint_id} rules fingerprint does not match the replayed Golden state")
...
def validate_loaded_model(model: object, descriptor: CheckpointDescriptor, mapping: GoldenProtocolMapping) -> GoldenPlayableModel:
    if not isinstance(model, GoldenPlayableModel):
        raise GenerationFailed(f"Golden checkpoint {descriptor.checkpoint_id} did not load as GoldenPlayableModel")
    if model.descriptor.checkpoint_id != descriptor.checkpoint_id:
        raise CheckpointIncompatible(f"Golden model descriptor does not match requested checkpoint {descriptor.checkpoint_id}")
    ...
    if descriptor.profile_id is not None and profile_value != descriptor.profile_id:
        raise CheckpointIncompatible(f"Golden model {descriptor.checkpoint_id} metadata mismatch for profile_id")
    profile_fingerprint = metadata.get("profile_fingerprint", metadata.get("training_profile_fingerprint"))
    if descriptor.profile_fingerprint is not None and profile_fingerprint != descriptor.profile_fingerprint:
        raise CheckpointIncompatible(f"Golden model {descriptor.checkpoint_id} metadata mismatch for profile_fingerprint")
    expected = {"architecture_id": descriptor.architecture_id, "rules_fingerprint": descriptor.rules_fingerprint, "observation_fingerprint": descriptor.observation_fingerprint, "target_fingerprint": descriptor.target_fingerprint, "komi": descriptor.komi}
    for key, value in expected.items():
        if value is not None and metadata.get(key) != value:
            raise CheckpointIncompatible(f"Golden model {descriptor.checkpoint_id} metadata mismatch for {key}")
    topology_fp = metadata.get("topology_fingerprint")
    if topology_fp != mapping.golden_topology_fingerprint:
        raise CheckpointIncompatible(f"Golden model {descriptor.checkpoint_id} topology fingerprint does not match mapping")
    if float(metadata.get("komi", math.nan)) != float(descriptor.komi):
        raise CheckpointIncompatible(f"Golden model {descriptor.checkpoint_id} komi does not match descriptor")
    return model


class GoldenMoveSelector:
    def __init__(self, *, mapping_resolver: Callable[[str, int], GoldenProtocolMapping] = mapping_for, search_factory=SequentialPUCT, adapter_factory=GoldenSearchAdapter, search_contract: GoldenInteractiveSearchContract = INTERACTIVE_SEARCH_CONTRACT) -> None:
        self.mapping_resolver = mapping_resolver
        self.search_factory = search_factory
        self.adapter_factory = adapter_factory
        self.search_contract = search_contract

    def select_move(self, *, state: GoldenState, descriptor: CheckpointDescriptor, model: GoldenPlayableModel, mcts_sims: int) -> MoveSelection:
        if state.is_terminal:
        ...
        position = GoldenPositionContract(topology=descriptor.topology, size=descriptor.size, rule_set=descriptor.rule_set, komi=descriptor.komi)
        validate_checkpoint_position_compatibility(position=position, state=state, descriptor=descriptor, mapping=mapping)
        playable = validate_loaded_model(model, descriptor, mapping)
        settings = self.search_contract.settings(mcts_sims)
        try:
            result: SearchResult = self.search_factory(settings, adapter=self.adapter_factory()).search(state, playable.evaluator, seed=0)
        ...
        return MoveSelection(action=result.action, legal_actions=tuple(result.legal_actions), simulations=result.simulations, implementation_id=result.implementation_id, search_profile_id=self.search_contract.profile_id)


__all__ = ["GOLDEN_INTERACTIVE_SEARCH_PROFILE_ID", "GOLDEN_PROTOCOL_RULESET", "GoldenInteractiveSearchContract", "GoldenMoveSelector", "GoldenPositionContract", "INTERACTIVE_SEARCH_CONTRACT", "MoveSelection", "initial_state_for_position", "mapping_for_position", "replay_action_history", "replay_protocol_moves_strict", "validate_checkpoint_position_compatibility", "validate_loaded_model"]
