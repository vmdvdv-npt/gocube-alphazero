"""Shared GoCube V3 training infrastructure.

This module is intentionally independent of the retired gocube.train
executable. It contains reusable Coach, validation, and configuration
components used by the pinned KataGo training entrypoints.
"""
...
from alphazero.search_contract import assert_search_contract


def parse_args():
    """Parse the small common CLI surface retained for compatibility tests."""

    parser = argparse.ArgumentParser(description="Configure GoCube V3 training")
...
class GoCubeCoach(Coach):
    ...
    def __init__(self, game_cls, nnet, args):
        search_mode = (
            args.get("search_utility_mode", "legacy")
            if hasattr(args, "get")
            else getattr(args, "search_utility_mode", "legacy")
        )
        assert_search_contract(game_cls, search_mode)
...
def build_base_training_args(cli):
    if cli.workers < 1:
...
def print_training_configuration(args):
    ...
    print(f"  komi = {args.gocube_komi:g}")
