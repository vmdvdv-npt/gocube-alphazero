# Training run

- Run: c4-overnight-preflight-20260906-003115
- Status: RUNNING
- Started: 2026-09-06T00:31:15+04:00
- Ended: —
- Exit code: —
- Source commit: 85c87a7cfd467a4d3f4b2844253fb63d746d672a
- Completed iteration manifests: 0
- Reports branch: training-reports

## Command

```bash
bash /tmp/preflight_c4_overnight.sh c4-t001-c4-c001
```

## Published to GitHub

- run.md — this status summary
- console-tail.log — last 2000 console lines
- manifests/ — compact completed iteration manifests

Full console output remains local at `training_reports/c4-overnight-preflight-20260906-003115/console.log`.
