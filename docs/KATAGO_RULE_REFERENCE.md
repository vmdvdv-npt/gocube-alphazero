...
| B5 | `cpp/game/boardhistory.cpp` | `BoardHistory::clear`, `whiteBonusScore`, `secondEncoreStartColors` | `alphazero/envs/gocube/katago_v3.py`, `selfplay_semantics.py` | `_boardhistory_clear_white_bonus`, `v3_state_from_board`, `rebase_cleanup_training_state` | setup score initialization, capture counters, explicit CLEANUP_1/CLEANUP_2 starts, second-cleanup colors | GoCube stores the offset as immutable state and maps capture counts from capturing-player order | `test_gocube_s1_scoring_contract.py`, `s1-setup-main-counterexample`, `s1-setup-cleanup1-captures`, `s1-setup-cleanup2-captures` |
